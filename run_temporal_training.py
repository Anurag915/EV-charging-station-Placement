#!/usr/bin/env python3
"""
Complete Temporal Training Script for EV Charging Station Placement
================================================================

This script runs the full temporal training with your actual environment
and compares temporal vs non-temporal performance.
"""
...
print("🚀 STARTING COMPLETE TEMPORAL TRAINING")
print("=" * 60)
...
try:
    ...
    # Create mock data
    demand_avg = np.random.rand(50, 50)
    landuse_r1 = np.random.rand(50, 50)
    stations_mask = np.zeros((50, 50))
    stations_distance = np.random.rand(50, 50) * 100

# ============================================================================
# 2. CREATE YOUR EXISTING ENVIRONMENT
# ============================================================================

print("\n2. Creating your existing EVChargingEnv...")

class EVChargingEnv:
    """Your existing EV charging environment"""
    ...
    def _calculate_reward(self, x, y):
        ...
        # Base reward from demand
        demand_reward = self.demand[y, x] if y < self.demand.shape[0] and x < self.demand.shape[1] else 0
        
        # Landuse suitability
        landuse_reward = self.landuse[y, x] if y < self.landuse.shape[0] and x < self.landuse.shape[1] else 0
        
        # Distance penalty (avoid clustering)
        distance_penalty = 0
        ...
        # Coverage bonus
        coverage_bonus = 0
        for px, py in self.placements:
            dist = np.sqrt((x - px)**2 + (y - py)**2)
            if dist <= 10:  # Within coverage
                coverage_bonus += 0.05
        
        total_reward = demand_reward + landuse_reward - distance_penalty + coverage_bonus
        return total_reward

# Create your environment
env = EVChargingEnv(demand_avg, landuse_r1, stations_mask, stations_distance)
print("✅ Environment created successfully")

# ============================================================================
# 3. ENHANCED TIME-AWARE A2C AGENT
# ============================================================================
...
# ============================================================================
# 4. TIME-AWARE ENVIRONMENT WRAPPER
# ============================================================================
...
# ============================================================================
# 6. RUN COMPARISON: ORIGINAL vs TEMPORAL
# ============================================================================
...
# ============================================================================
# 7. RUN FULL TEMPORAL TRAINING
# ============================================================================

print("\n6. Running Full Temporal Training (100 episodes)...")
...
training_rewards = []
training_losses = []
temporal_rewards_detailed = []

print("   Training temporal agent for 100 episodes...")
for episode in range(100):
    reward, loss = train_temporal_episode(enhanced_agent, time_aware_env, optimizer, max_steps=50)
    training_rewards.append(reward)
    training_losses.append(loss)
    temporal_rewards_detailed.append(reward)
    
    if (episode + 1) % 20 == 0:
        avg_reward = np.mean(training_rewards[-20:])
        avg_loss = np.mean(training_losses[-20:])
...
os.makedirs('temporal_results', exist_ok=True)
...
axes[0, 0].plot(pd.Series(training_rewards).rolling(10).mean(), 'r-', linewidth=2)
axes[0, 0].set_title('Temporal Training Rewards', fontsize=14, fontweight='bold')
...
axes[0, 1].plot(pd.Series(training_losses).rolling(10).mean(), 'b-', linewidth=2)
axes[0, 1].set_title('Temporal Training Losses', fontsize=14, fontweight='bold')
axes[0, 1].set_xlabel('Episode')
axes[0, 1].set_ylabel('Loss')
axes[0, 1].grid(True, alpha=0.3)

# Performance comparison
axes[1, 0].bar(['Original', 'Temporal'], [original_avg, temporal_avg], 
               color=['lightblue', 'lightgreen'], alpha=0.7)
axes[1, 0].set_title('Performance Comparison', fontsize=14, fontweight='bold')
axes[1, 0].set_ylabel('Average Reward')
axes[1, 0].text(0, original_avg + 0.01, f'{original_avg:.3f}', ha='center')
axes[1, 0].text(1, temporal_avg + 0.01, f'{temporal_avg:.3f}', ha='center')

# Temporal patterns
time_multipliers = time_aware_env.time_multipliers
hours = list(time_multipliers.keys())
multipliers = list(time_multipliers.values())

axes[1, 1].plot(hours, multipliers, 'b-', linewidth=2, marker='o')
axes[1, 1].set_title('Delhi Temporal Patterns', fontsize=14, fontweight='bold')
axes[1, 1].set_xlabel('Hour of Day')
axes[1, 1].set_ylabel('Demand Multiplier')
axes[1, 1].grid(True, alpha=0.3)

# Highlight rush hours
rush_hours = [h for h in hours if 7 <= h <= 10 or 17 <= h <= 20]
rush_multipliers = [time_multipliers[h] for h in rush_hours]
axes[1, 1].scatter(rush_hours, rush_multipliers, color='red', s=100, 
                  label='Rush Hours', zorder=5)
axes[1, 1].legend()

plt.tight_layout()
plt.savefig('temporal_results/complete_training_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
print("\n8. Analyzing Time Period Performance...")
...
for period_name, hour in time_periods.items():
    ...
    for step in range(30):
        with torch.no_grad():
            action_mean, value, reward_prob, hidden = enhanced_agent(
                torch.FloatTensor(spatial_state).unsqueeze(0),
                torch.FloatTensor(temporal_state).unsqueeze(0).unsqueeze(0)
            )
        ...
        spatial_state, temporal_state, reward, done, info = time_aware_env.step(
...
plt.tight_layout()
plt.savefig('temporal_results/time_period_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
results = {
    ...
    'time_period_rewards': period_rewards,
    'training_episodes': len(training_rewards)
}

with open('temporal_results/training_results.json', 'w') as f:
...
}, 'temporal_results/enhanced_temporal_model.pth')

print("✅ Results and model saved to temporal_results/")
...
print(f"✅ Full temporal training completed (100 episodes)")
print(f"✅ Time period analysis completed")

print("\n🎯 UNIQUE FEATURES DEMONSTRATED:")
print("• LSTM learns Delhi-specific temporal patterns")
...
print("\n📊 GENERATED FILES:")
print("• temporal_results/complete_training_analysis.png")
print("• temporal_results/time_period_analysis.png")
print("• temporal_results/training_results.json")
print("• temporal_results/enhanced_temporal_model.pth")

print("\n🚀 PERFORMANCE SUMMARY:")
print(f"   Original Environment: {original_avg:.3f} avg reward")
print(f"   Temporal Environment: {temporal_avg:.3f} avg reward")
print(f"   Improvement: {improvement:+.1f}%")
